...
func (self *Sender) SendVerifyTxReq(verifyType vtypes.VerifyType, tx *ctypes.Transaction) bool {
...
func (self *Sender) sendVerifyStatelessTxReq(tx *ctypes.Transaction) bool {
...
func (self *Sender) sendVerifyStatefulTxReq(tx *ctypes.Transaction) bool {
	req := &vtypes.VerifyTxReq{
		Tx: *tx,
	}
	rspPid := self.txPoolActor
...
func (self *Sender) HandleRegisterValidator(pid *actor.PID, tpe vtypes.VerifyType, id string) {
	self.validatorActor.Lock()
	defer self.validatorActor.Unlock()
...
func (self *Sender) HandleUnRegisterValidator(verifyType vtypes.VerifyType,
	id string) {
